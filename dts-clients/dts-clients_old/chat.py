...
def main():
    ...
    argp.add_argument('-q' , '--QoS'      , action='store_true',           help='require Quality of Service aware routing')
    ...
    argp.add_argument('-vc', '--client'   , const='True',        nargs='?',help='start a ffmpeg video client on this host')
    ...
    if not args.client:
        ...
        if args.client == 'True':
            ...
            os.system('ffmpeg -re -i etcp:{}:{} -f mpegts -vcodec mpeg4 -strict -2 -acodec ac3 -ac 2 -ab 128k -r 30 -b:v 500k -threads 2 {}'
                      .format(''.join(["%02x" % ord(x) for x in hash_title]).strip(), iface, args.client))
